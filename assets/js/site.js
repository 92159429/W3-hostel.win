const translations = {
  "zh-Hant": {
    nav: ["首頁", "W3", "空間與設施", "旅遊資訊", "聯絡與交通"],
    home: {
      metaTitle: "羅東住宿推薦｜W3 青旅・宜蘭青年旅館｜近羅東夜市・車站",
      metaDescription: "W3 青旅位於宜蘭羅東核心地段，鄰近羅東夜市與車站，交通便利、生活機能完整，提供清淨舒適的住宿空間，適合自由行、背包客與團體旅客。",
      ogTitle: "W3 Hostel Luodong｜Clean & Quiet Stay",
      ogDescription: "Night Market nearby, full delivery coverage, and a flexible white-aesthetic stay in Luodong.",
      eyebrow: "羅東住宿推薦｜宜蘭青年旅館",
      title: "在羅東，找到剛剛好的停留方式。",
      desc: "W3 青旅位於宜蘭羅東核心地段，鄰近羅東夜市、火車站與完整生活機能。結合便利外送、靜謐空間與舒適住宿體驗，無論自由行、背包客或短期停留，都能輕鬆享受更自在、更有效率的羅東旅程。",
      cta1: "Agoda 訂房",
      cta2: "Booking.com 預約",
      sec1k: "W3 住宿體驗",
      sec1t: "入住 W3，感受剛剛好的旅宿節奏",
      sec1d: "用最短時間理解 W3 最有感的三件事：地點、美食與空間。",
      f1t: "核心地段・步行生活圈",
      f1d: "羅東夜市、臺鐵羅東站、轉運站、便利商店與各式餐飲皆近在咫尺，讓移動與覓食更輕鬆。",
      f2t: "外送完整覆蓋",
      f2d: "Uber Eats 與 foodpanda 全區支援，從人氣小吃到宵夜美食，都能快速送達。",
      f3t: "乾淨安靜的舒適空間",
      f3d: "明亮潔淨、安靜放鬆的環境，適合休息、閱讀、工作，或單純沉浸在自己的旅行節奏中。",
      sec2k: "住宿方式",
      sec2t: "你的旅程，由你定義。",
      sec2d: "在 W3，你不必被固定行程綁住。白天自由探索羅東，晚上輕鬆享用外送美食，想休息時就回到舒服乾淨的空間。",
      x1t: "短期差旅",
      x1d: "適合短暫工作、行程整理、回覆訊息與靜心閱讀。",
      x2t: "輕鬆享食",
      x2d: "不用排隊、不必出門，在房內或公共空間也能輕鬆享受羅東人氣美食。",
      x3t: "自在放鬆",
      x3d: "不趕行程、不被打擾，讓旅程回到舒服、自然的步調。",
      sec3k: "地理優勢",
      sec3t: "住在羅東核心，一切更簡單。",
      sec3d: "交通、餐飲與生活機能集中，讓你的每一次移動與選擇都更省時省力。",
      l1t: "羅東轉運站",
      l1d: "車程約 3 分鐘｜步行約 13 分鐘",
      l2t: "臺鐵羅東站",
      l2d: "車程約 5 分鐘｜步行約 18 分鐘",
      l3t: "羅東觀光夜市",
      l3d: "車程約 6 分鐘｜步行約 20 分鐘以上",
      sec4k: "立即預訂",
      sec4t: "準備好入住羅東了嗎？",
      sec4d: "立即查看房況與價格，選擇最適合你的訂房平台，開啟宜蘭旅程。",
      book1: "Agoda 查看房況",
      book1d: "快速查詢房況、日期與價格。",
      book2: "Booking.com 立即預訂",
      book2d: "直接前往預訂頁完成訂房。"
    },

    about: {
      metaTitle: "羅東 W3 青旅｜宜蘭住宿・交通便利・外送完整",
      metaDescription: "W3 青旅位於羅東核心生活圈，交通便利、生活機能完善，外送服務完整，提供安靜舒適的住宿環境。",
      ogTitle: "羅東 W3｜在城市中找到你的節奏與空間",
      ogDescription: "交通完整、生活便利、外送強，讓你在羅東找到能商務、能放鬆、也能安靜休息的空間。",
      heroTitle: "羅東 W3｜在城市中找到你的節奏與空間",
      heroDesc: "位於羅東核心生活圈，交通完整、生活便利、外送強，讓你在城市中自然切換工作、休息與放鬆節奏。",
      p1t: "地點優勢｜交通完整",
      p1d: "羅東火車站約 5–8 分鐘車程／步行約 15 分鐘；羅東轉運站約 5 分鐘車程／步行約 12–15 分鐘。從台北可搭火車或客運直達，轉乘即可快速抵達。",
      p2t: "生活機能｜完整生活圈",
      p2d: "藏壽司、寶雅、全聯皆在附近，7-11 與全家便利商店密集，早餐店、餐廳與飲料店步行可達，吃的、用的、補給都很方便。",
      p3t: "外送優勢｜核心主打",
      p3d: "Uber Eats 與 foodpanda 全區覆蓋，羅東夜市與熱門餐廳幾乎都可送達，不用排隊、不用出門，在空間內就能輕鬆吃遍羅東。",
      c1t: "地點優勢（交通完整）",
      c1i1: "羅東火車站：約 5–8 分鐘車程／步行約 15 分鐘",
      c1i2: "羅東轉運站：約 5 分鐘車程／步行約 12–15 分鐘",
      c1i3: "台北可搭火車或客運直達，轉乘即可快速抵達",
      c2t: "生活機能（完整生活圈）",
      c2i1: "藏壽司、寶雅、全聯皆在附近",
      c2i2: "7-11／全家便利商店密集",
      c2i3: "早餐店、餐廳、飲料店步行可達",
      c2i4: "吃的、用的、補給全部不用煩惱",
      c3t: "外送優勢（核心主打）",
      c3i1: "Uber Eats／foodpanda 全區覆蓋",
      c3i2: "羅東夜市與熱門餐廳幾乎全部可送達",
      c3i3: "不用排隊、不用出門",
      c3i4: "在空間內就能吃遍羅東"
    },

    space: {
      metaTitle: "空間與設施｜羅東住宿 W3 青旅",
      metaDescription: "提供舒適床位、免費洗衣、開放式廚房與閱讀空間，打造乾淨安靜且便利的羅東住宿體驗。",
      ogTitle: "空間與設施｜羅東 W3",
      ogDescription: "可商務、可放鬆、可私密、可安靜的多功能住宿空間。",
  heroTitle: "空間與設施",
  heroDesc: "從住宿形式到生活便利、盥洗整潔、公共空間與停車安排，羅東 W3 提供能自由切換狀態的住宿空間。",
  p1t: "住宿形式",
  p1d: "床位設計提供個人休息的舒適與基本隱私，讓旅途中也能保有自己的節奏。",
  p2t: "生活便利",
  p2d: "提供免費洗衣機與日光曬衣區，通風良好，讓旅途中清洗與整理衣物更方便。",
  p3t: "公共空間",
  p3d: "開放式廚房與中島，備有雙口 IH 感應爐、微波爐與烤箱，可供食物加熱與簡單料理。閱讀區域相對安靜、不易被打擾，讓人自在停留。",
  p4t: "空間定位",
  p4d: "可商務、可放鬆、可私密、可安靜，讓你依照當下狀態自由切換空間使用方式。",
  galleryTitle: "空間相簿",
  galleryDesc: "在光影與留白之間，感受 W3 的空間節奏。"
},

  travel: {
      metaTitle: "공간 및 시설｜뤄동 W3 호스텔",
      metaDescription: "편안한 침대, 무료 세탁, 공용 주방과 조용한 공간으로 쾌적한 숙박 환경을 제공합니다.",
  ogTitle: "旅遊資訊｜羅東 W3",
  ogDescription: "從羅東夜市到自然景點與四季活動，入住 W3 輕鬆規劃宜蘭旅程。",
  heroTitle: "旅遊資訊",
  heroDesc: "從附近景點、季節活動到自然延伸旅遊，讓你更輕鬆掌握羅東與宜蘭的旅行節奏。",
  p1t: "附近景點｜觀光熱區",
  p1d: "入住 W3，可輕鬆串聯羅東熱門景點與生活圈，白天漫步、夜晚逛夜市，感受在地節奏。",
  p2t: "季節活動",
  p2d: "從春季櫻花、夏日河濱，到秋季稻浪與各式節慶活動，四季都有不同風景。",
  p3t: "探索宜蘭自然風景",
  p3d: "森林、冷泉、地熱與山林景觀，讓羅東成為延伸自然旅程的最佳起點。",

  p4t: "鄰近景點",
  p4c1n: "羅東觀光夜市",
  p4c1m: "🚖 6 分鐘・🚶 20 分鐘",
  p4c2n: "羅東林業文化園區",
  p4c2m: "🚖 7 分鐘・🚶 28 分鐘",
  p4c3n: "冬山河親水公園",
  p4c3m: "🚖 9 分鐘",
  p4c4n: "羅東運動公園",
  p4c4m: "🚖 12 分鐘",
  p4c5n: "宜蘭傳藝園區",
  p4c5m: "🚖 12 分鐘",

  p5t: "季節活動嘉年華",
  p5c1n: "🌸 羅莊櫻花季",
  p5c1m: "每年 2 月中至 3 月初，羅莊櫻花步道，🚖 2 分鐘・🚶 10 分鐘",
  p5c2n: "🌾 羅東稻穗節",
  p5c2m: "每年 10 至 11 月，羅東周邊農田，🚶 約 30 秒",
  p5c3n: "🌿 伯朗大道",
  p5c3m: "全年適合前往，191 縣道，🚖 約 10 分鐘",
  p5c4n: "🎆 村却國際溫泉酒店跨年煙火",
  p5c4m: "每年 12 月 31 日，🚖 3 分鐘・🚶 11 分鐘",

  p6t: "世界級景點",
  p6c1n: "太平山國家森林遊樂區",
  p6c1m: "🚖 90 分鐘",
  p6c2n: "蘇澳冷泉公園",
  p6c2m: "🚖 21 分鐘"
},

    contact: {
      metaTitle: "聯絡與交通｜羅東 W3 青旅",
      metaDescription: "查看 W3 青旅地址、電話與交通方式，輕鬆抵達羅東住宿。",
      ogTitle: "聯絡與交通｜羅東 W3",
      ogDescription: "查看 W3 Hostel 的電話、地址、交通方式與地圖導航資訊。",
      title: "聯絡與交通",
      desc: "無論搭車或自行駕車前來，都能輕鬆找到 W3 青旅。也歡迎透過電話、Email 或社群與我們聯繫。",
      blendTitle: "W3 青旅",
      blendDesc: "265033 宜蘭縣羅東鎮羅莊北街 13 號，適合搭乘大眾運輸或自行開車前往。",
      cardTitle: "聯絡方式",
      phone: "聯絡電話",
      address: "地址",
      email: "電子郵件",
      transport: "大眾運輸方式",
      drive: "自行駕車",
      transportDesc: "自羅東車站或轉運站步行約 8–10 分鐘即可抵達。",
      driveDesc: "可經國道 5 號銜接宜蘭地區道路，依導航前往宜蘭縣羅東鎮羅莊北街 13 號。",
      map: "地圖瀏覽",
      nav: "開車導航"
    },

    footer: { rights: "W3 青旅版權所有 © 2026" },

    booking: {
      title: "快捷服務",
      agoda: "Agoda 訂房",
      booking: "Booking.com 訂房",
      contact: "聯絡我們",
      phone: "立即撥打",
      line: "LINE 諮詢",
      ig: "Instagram"
    }
  },

  "en": {
    nav: ["Home", "W3", "Spaces & Facilities", "Travel Info", "Contact & Access"],
    home: {
      metaTitle: "W3 Hostel Luodong｜Yilan Hostel Near Night Market & Station",
      metaDescription: "W3 Hostel in Luodong, Yilan is located near Luodong Night Market and the train station, offering a clean, quiet, and convenient stay for solo travelers, backpackers, and groups.",
      ogTitle: "W3 Hostel Luodong｜Clean & Quiet Stay",
      ogDescription: "Night Market nearby, full delivery coverage, and a flexible white-aesthetic stay in Luodong.",
      eyebrow: "Luodong Hostel in Yilan",
      title: "Find the stay rhythm that fits Luodong best.",
      desc: "W3 Hostel is located in central Luodong, close to Luodong Night Market, the train station, and everyday conveniences. With easy food delivery, a quiet atmosphere, and a clean, comfortable setting, it’s a great base for solo travelers, short stays, and trips with friends.",
      cta1: "Book on Agoda",
      cta2: "Reserve on Booking.com",
      sec1k: "The W3 Experience",
      sec1t: "Why stay at W3?",
      sec1d: "Discover the three things that matter most here: location, food access, and comfort.",
      f1t: "Central Location",
      f1d: "Stay close to Luodong Night Market, the train station, the bus hub, convenience stores, and local dining options.",
      f2t: "Full Delivery Coverage",
      f2d: "Uber Eats and foodpanda both cover the area, so popular Luodong meals and late-night bites are easy to enjoy.",
      f3t: "Clean and Quiet Comfort",
      f3d: "A bright, tidy, and calming space for rest, reading, remote work, or simply slowing down.",
      sec2k: "How You Stay",
      sec2t: "Your trip, your pace.",
      sec2d: "At W3, you do not have to follow a fixed routine. Explore Luodong by day, enjoy delivery at night, and return to a calm, comfortable space whenever you want to rest.",
      x1t: "Short Business Stays",
      x1d: "A good fit for light work, itinerary planning, messaging, and quiet reading.",
      x2t: "Easy Dining",
      x2d: "No lines and no need to go out. Enjoy Luodong’s popular food right in your room or shared area.",
      x3t: "Relax Naturally",
      x3d: "No rush, no pressure, no interruptions—just a stay that matches your own rhythm.",
      sec3k: "Location Advantage",
      sec3t: "Stay in the heart of Luodong and keep things simple.",
      sec3d: "Transport, dining, and everyday essentials are all nearby, making every move and decision easier.",
      l1t: "Luodong Bus Station",
      l1d: "About 3 minutes by car｜About 13 minutes on foot",
      l2t: "Luodong Train Station",
      l2d: "About 5 minutes by car｜About 18 minutes on foot",
      l3t: "Luodong Night Market",
      l3d: "About 6 minutes by car｜About 20+ minutes on foot",
      sec4k: "Book Now",
      sec4t: "Ready to stay in Luodong?",
      sec4d: "Check rates and availability now, then choose the booking platform that suits you best.",
      book1: "Check Agoda Availability",
      book1d: "Quickly view room status, dates, and rates.",
      book2: "Book on Booking.com",
      book2d: "Go straight to the booking page and complete your reservation."
    },

    about: {
      metaTitle: "W3 Hostel Luodong｜Central Location & Convenient Stay in Yilan",
      metaDescription: "Located in central Luodong, W3 Hostel offers convenient transport, strong local amenities, and full delivery coverage in a quiet and comfortable setting.",
      ogTitle: "W3 Luodong｜Find Your Rhythm and Space in the City",
      ogDescription: "Convenient transport, strong daily amenities, and full delivery coverage in the heart of Luodong.",
      heroTitle: "W3 Luodong｜Find Your Rhythm and Space in the City",
      heroDesc: "Located in Luodong’s core living district, W3 combines transport convenience, everyday amenities, and full delivery access so you can move naturally between work, rest, and downtime.",
      p1t: "Location Advantage｜Convenient Transport",
      p1d: "Luodong Train Station is about 5–8 minutes away by car or around 15 minutes on foot. Luodong Bus Station is about 5 minutes by car or around 12–15 minutes on foot. Direct trains and coaches from Taipei make arrival easy.",
      p2t: "Daily Convenience｜Everything Nearby",
      p2d: "Sushiro, POYA, PX Mart, 7-Eleven, FamilyMart, breakfast shops, restaurants, and drink stores are all close by, making meals and supplies easy.",
      p3t: "Delivery Advantage｜A Key Highlight",
      p3d: "Uber Eats and foodpanda fully cover the area, and most popular Luodong restaurants and Night Market food can be delivered directly to you.",
      c1t: "Location Advantage",
      c1i1: "Luodong Train Station: about 5–8 minutes by car / about 15 minutes on foot",
      c1i2: "Luodong Bus Station: about 5 minutes by car / about 12–15 minutes on foot",
      c1i3: "Easy access from Taipei by direct train or coach",
      c2t: "Daily Convenience",
      c2i1: "Sushiro, POYA, and PX Mart are all nearby",
      c2i2: "Dense coverage of 7-Eleven and FamilyMart",
      c2i3: "Breakfast shops, restaurants, and drink stores within walking distance",
      c2i4: "Food, essentials, and supplies are all easy to get",
      c3t: "Delivery Advantage",
      c3i1: "Full Uber Eats and foodpanda coverage",
      c3i2: "Most popular Luodong meals can be delivered",
      c3i3: "No lines and no need to go out",
      c3i4: "Enjoy Luodong food without leaving the space"
    },

    space: {
      metaTitle: "Spaces & Facilities｜W3 Hostel Luodong",
      metaDescription: "Comfortable beds, free laundry, open kitchen, and quiet shared spaces create a clean and relaxing stay in Luodong.",
  ogTitle: "Spaces & Facilities｜W3 Luodong",
  ogDescription: "A flexible stay space for work, rest, privacy, and quiet time.",
  heroTitle: "Spaces & Facilities",
  heroDesc: "From sleeping arrangements and daily convenience to shared areas and parking, W3 Luodong offers a stay space that adapts to different travel needs.",
  p1t: "Stay Setup",
  p1d: "Bed spaces are designed to offer comfort and a basic sense of privacy, helping you keep your own rhythm while traveling.",
  p2t: "Daily Convenience",
  p2d: "Free laundry machines and a sun-drying area make it easier to wash and organize clothes during your stay.",
  p3t: "Shared Space",
  p3d: "The open kitchen and island area include a double-burner IH cooktop, microwave, and oven for simple meal prep and reheating. The reading area stays relatively quiet and comfortable.",
  p4t: "Space Identity",
  p4d: "Suitable for business, relaxation, privacy, and quiet time, so you can shift the way you use the space based on how you feel.",
  galleryTitle: "Gallery",
  galleryDesc: "Experience W3 through light, space, and quiet details from different perspectives."
},

travel: {
      mmetaTitle: "Luodong Travel Guide｜Yilan Attractions & Night Market",
      metaDescription: "Explore Luodong Night Market, nearby attractions, and Yilan nature spots for an easy and enjoyable travel plan.",
  ogTitle: "Travel Info｜W3 Luodong",
  ogDescription: "From local hotspots to seasonal events and scenic escapes, plan your Yilan stay with ease.",
  heroTitle: "Travel Info",
  heroDesc: "From nearby attractions and seasonal highlights to nature escapes, it is easy to plan your Luodong and Yilan itinerary from W3.",
  p1t: "Nearby Attractions",
  p1d: "Staying at W3 makes it easy to explore Luodong’s popular spots and local daily-life areas, from daytime walks to lively night-market evenings.",
  p2t: "Seasonal Highlights",
  p2d: "From cherry blossoms in spring and riverside scenery in summer to golden rice fields and festivals in autumn, each season offers a different rhythm.",
  p3t: "Explore Yilan Nature",
  p3d: "Forests, cold springs, geothermal landscapes, and mountain scenery make Luodong a convenient starting point for deeper nature trips.",

  p4t: "Nearby Spots",
  p4c1n: "Luodong Night Market",
  p4c1m: "🚖 6 min ・ 🚶 20 min",
  p4c2n: "Luodong Forestry Culture Park",
  p4c2m: "🚖 7 min ・ 🚶 28 min",
  p4c3n: "Dongshan River Water Park",
  p4c3m: "🚖 9 min",
  p4c4n: "Luodong Sports Park",
  p4c4m: "🚖 12 min",
  p4c5n: "Yilan Traditional Arts Center",
  p4c5m: "🚖 12 min",

  p5t: "Seasonal Carnival",
  p5c1n: "🌸 Luozhuang Cherry Blossom Season",
  p5c1m: "Mid-February to early March each year, Luozhuang Cherry Blossom Trail, 🚖 2 min ・ 🚶 10 min",
  p5c2n: "🌾 Luodong Rice Ear Festival",
  p5c2m: "October to November each year, nearby Luodong fields, 🚶 about 30 sec",
  p5c3n: "🌿 Brown Boulevard",
  p5c3m: "Great year-round, County Road 191, 🚖 about 10 min",
  p5c4n: "🎆 Cuncyue International Hot Spring Hotel New Year Fireworks",
  p5c4m: "Every December 31, 🚖 3 min ・ 🚶 11 min",

  p6t: "Extended Nature Trips",
  p6c1n: "Taipingshan National Forest Recreation Area",
  p6c1m: "🚖 90 min",
  p6c2n: "Suao Cold Spring Park",
  p6c2m: "🚖 21 min"
},
    contact: {
      metaTitle: "Contact & Access｜W3 Hostel Luodong",
      metaDescription: "Find W3 Hostel’s address, phone number, and directions for easy access in Luodong.",
      ogTitle: "Contact & Access｜W3 Luodong",
      ogDescription: "View W3 Hostel’s phone, address, transport information, and map navigation details.",
      title: "Contact & Access",
      desc: "Whether you arrive by transit or by car, W3 Hostel is easy to find. You are also welcome to contact us by phone, email, or social media.",
      blendTitle: "W3 Hostel",
      blendDesc: "No. 13, Luozhuang N. St., Luodong Township, Yilan County. Easy to reach by public transport or by car.",
      cardTitle: "Contact",
      phone: "Phone",
      address: "Address",
      email: "Email",
      transport: "Public Transport",
      drive: "Driving",
      transportDesc: "About an 8–10 minute walk from Luodong Train Station or Luodong Bus Station.",
      driveDesc: "Take National Freeway 5 into Yilan and follow navigation to No. 13, Luozhuang N. St., Luodong Township.",
      map: "View Map",
      nav: "Driving Navigation"
    },

    footer: { rights: "Copyright © 2026 W3 Hostel. All Rights Reserved." },

    booking: {
      title: "Quick Access",
      agoda: "Book on Agoda",
      booking: "Book on Booking.com",
      contact: "Contact Us",
      phone: "Call Now",
      line: "LINE Chat",
      ig: "Instagram"
    }
  },

  "ja": {
    nav: ["ホーム", "W3", "空間と設備", "観光情報", "連絡先とアクセス"],
    home: {
      metaTitle: "羅東 宿泊｜W3 ホステル・宜蘭｜夜市・駅近の快適な宿",
      metaDescription: "W3 Hostelは宜蘭・羅東中心に位置し、羅東夜市や駅に近く、清潔で快適な滞在を提供します。一人旅からグループ旅行まで幅広く対応。",
      ogTitle: "W3 Hostel Luodong｜Clean & Quiet Stay",
      ogDescription: "Night Market nearby, full delivery coverage, and a flexible white-aesthetic stay in Luodong.",
      eyebrow: "羅東のおすすめ宿泊｜宜蘭ホステル",
      title: "羅東で、ちょうどいい滞在を見つける。",
      desc: "W3 Hostelは羅東の中心エリアにあり、夜市、駅、生活施設へのアクセスが良好です。デリバリーの便利さ、静かで清潔な空間、快適な滞在環境を兼ね備え、一人旅や短期滞在、友人との旅行にも適しています。",
      cta1: "Agodaで予約",
      cta2: "Booking.comで予約",
      sec1k: "W3の滞在体験",
      sec1t: "W3を選ぶ理由",
      sec1d: "立地、食事の便利さ、快適な空間。W3の魅力を3つのポイントで紹介します。",
      f1t: "中心立地・徒歩生活圏",
      f1d: "羅東夜市、駅、バスターミナル、コンビニ、飲食店が近く、移動も食事もスムーズです。",
      f2t: "デリバリー対応が充実",
      f2d: "Uber Eats と foodpanda が利用でき、人気の羅東グルメや夜食も気軽に楽しめます。",
      f3t: "清潔で静かな快適空間",
      f3d: "明るく整った空間で、休憩、読書、仕事、気分転換まで心地よく過ごせます。",
      sec2k: "滞在スタイル",
      sec2t: "旅の過ごし方は、あなた次第。",
      sec2d: "昼は羅東を自由に巡り、夜はデリバリーを楽しみ、疲れたら静かで心地よい空間に戻る。そんな自然な滞在ができます。",
      x1t: "短期ワーク滞在",
      x1d: "軽い仕事、旅程整理、メッセージ対応、読書にも向いています。",
      x2t: "気軽に食事",
      x2d: "並ばず、出かけず、部屋や共有スペースで羅東の人気グルメを楽しめます。",
      x3t: "自然にくつろぐ",
      x3d: "急がず、無理せず、自分らしいリズムで過ごせます。",
      sec3k: "立地の魅力",
      sec3t: "羅東の中心に泊まれば、すべてがもっと簡単に。",
      sec3d: "交通、食事、日常の利便性がそろい、移動も判断もスムーズになります。",
      l1t: "羅東バスターミナル",
      l1d: "車で約3分｜徒歩約13分",
      l2t: "羅東駅",
      l2d: "車で約5分｜徒歩約18分",
      l3t: "羅東夜市",
      l3d: "車で約6分｜徒歩約20分以上",
      sec4k: "今すぐ予約",
      sec4t: "羅東での滞在を始めませんか？",
      sec4d: "空室と料金を確認して、使いやすい予約サイトからそのまま予約できます。",
      book1: "Agodaで空室確認",
      book1d: "日程・空室・料金をすばやく確認。",
      book2: "Booking.comで予約",
      book2d: "予約ページへ進んで手続きを完了。"
    },

    about: {
      metaTitle: "羅東 W3｜宜蘭 宿泊・交通便利・快適な滞在",
      metaDescription: "羅東中心に位置するW3は、交通や生活の利便性に優れ、静かで快適な宿泊環境を提供します。",
      ogTitle: "羅東 W3｜街の中で、自分のリズムと空間を見つける",
      ogDescription: "交通・生活利便・デリバリー対応がそろった羅東中心の快適な滞在拠点。",
      heroTitle: "羅東 W3｜街の中で、自分のリズムと空間を見つける",
      heroDesc: "羅東の中心生活圏に位置し、交通の便利さ、生活機能、デリバリーのしやすさを兼ね備え、仕事・休息・くつろぎを自然に切り替えられる滞在拠点です。",
      p1t: "立地の魅力｜交通アクセス",
      p1d: "羅東駅から車で約5〜8分、徒歩約15分。羅東バスターミナルからは車で約5分、徒歩約12〜15分です。台北からは直通の鉄道やバスでアクセスしやすい立地です。",
      p2t: "生活機能｜そろった周辺環境",
      p2d: "くら寿司、POYA、PX Mart、7-Eleven、FamilyMart、朝食店、レストラン、ドリンク店が近く、食事や買い物に困りません。",
      p3t: "デリバリーの強み｜大きな魅力",
      p3d: "Uber Eats と foodpanda がエリアを広くカバーしており、羅東夜市や人気店の料理も部屋まで届けてもらえます。",
      c1t: "立地の魅力",
      c1i1: "羅東駅：約5〜8分（車）／約15分（徒歩）",
      c1i2: "羅東バスターミナル：約5分（車）／約12〜15分（徒歩）",
      c1i3: "台北から鉄道・バスでアクセスしやすい",
      c2t: "生活機能",
      c2i1: "くら寿司、POYA、PX Martが近い",
      c2i2: "7-Eleven と FamilyMart が多い",
      c2i3: "朝食店、飲食店、ドリンク店が徒歩圏内",
      c2i4: "食事・日用品・補給がしやすい",
      c3t: "デリバリーの強み",
      c3i1: "Uber Eats／foodpanda に対応",
      c3i2: "羅東の人気店の料理を配達可能",
      c3i3: "並ばず、外出せずに楽しめる",
      c3i4: "宿の中で羅東グルメを満喫できる"
    },

    space: {
      metaTitle: "空間と設備｜羅東 W3 ホステル",
      metaDescription: "快適なベッド、無料洗濯、キッチンと読書スペースを備えた清潔で落ち着いた宿泊空間です。",
      ogTitle: "空間と設備｜羅東 W3",
  ogDescription: "仕事にも休息にも、静かな時間にも合う多機能な宿泊空間。",
  heroTitle: "空間と設備",
  heroDesc: "宿泊スタイルから生活の便利さ、共有空間、駐車まで、W3は滞在中のさまざまな過ごし方に対応する空間を整えています。",
  p1t: "宿泊スタイル",
  p1d: "ベッドスペースは、快適さと基本的なプライバシーを確保し、旅の中でも自分のリズムを保てるよう設計されています。",
  p2t: "生活の便利さ",
  p2d: "無料の洗濯機と日当たりの良い物干しエリアがあり、旅の途中でも衣類の洗濯と整理がしやすくなっています。",
  p3t: "共有空間",
  p3d: "オープンキッチンとアイランドには、IHコンロ、電子レンジ、オーブンを備え、簡単な調理や温め直しが可能です。読書エリアは比較的静かで落ち着いて過ごせます。",
  p4t: "空間の位置づけ",
  p4d: "仕事、リラックス、プライベート、静かな時間のどれにも対応し、その時の気分に合わせて使い方を変えられます。",
  galleryTitle: "ギャラリー",
  galleryDesc: "光と余白のあいだで、W3の空間の流れをご覧ください。"
},

travel: {
      metaTitle: "羅東 観光｜宜蘭 観光スポット・夜市",
      metaDescription: "羅東夜市や周辺観光地、宜蘭の自然スポットを紹介し、旅行計画をサポートします。",
  ogTitle: "観光情報｜羅東 W3",
  ogDescription: "夜市から季節イベント、自然スポットまで、W3を拠点に宜蘭の旅を組み立てやすくなります。",
  heroTitle: "観光情報",
  heroDesc: "周辺スポット、季節イベント、自然観光まで、羅東と宜蘭の旅をよりスムーズに計画できます。",
  p1t: "周辺観光スポット",
  p1d: "W3を拠点にすると、羅東の人気スポットやローカルエリアを気軽に楽しめます。",
  p2t: "季節の見どころ",
  p2d: "春の桜、夏の川辺、秋の稲穂や行事など、季節ごとに異なる魅力があります。",
  p3t: "宜蘭の自然を巡る",
  p3d: "森林、冷泉、地熱、山々の風景など、自然を満喫できる場所がそろっています。",

  p4t: "近隣スポット",
  p4c1n: "羅東夜市",
  p4c1m: "🚖 6分 ・ 🚶 20分",
  p4c2n: "羅東林業文化園区",
  p4c2m: "🚖 7分 ・ 🚶 28分",
  p4c3n: "冬山河親水公園",
  p4c3m: "🚖 9分",
  p4c4n: "羅東運動公園",
  p4c4m: "🚖 12分",
  p4c5n: "宜蘭伝統芸術センター",
  p4c5m: "🚖 12分",

  p5t: "季節イベントカーニバル",
  p5c1n: "🌸 羅莊桜花シーズン",
  p5c1m: "毎年2月中旬〜3月初旬、羅莊桜花トレイル、🚖 2分 ・ 🚶 10分",
  p5c2n: "🌾 羅東稲穂節",
  p5c2m: "毎年10〜11月、羅東周辺の田園地帯、🚶 約30秒",
  p5c3n: "🌿 伯朗大道",
  p5c3m: "通年おすすめ、191県道、🚖 約10分",
  p5c4n: "🎆 村却国際温泉ホテル年越し花火",
  p5c4m: "毎年12月31日、🚖 3分 ・ 🚶 11分",

  p6t: "自然スポットをさらに巡る",
  p6c1n: "太平山国家森林遊楽区",
  p6c1m: "🚖 90分",
  p6c2n: "蘇澳冷泉公園",
  p6c2m: "🚖 21分"
},

    contact: {
      metaTitle: "連絡とアクセス｜羅東 W3 ホステル",
      metaDescription: "住所、電話番号、アクセス方法を確認して、スムーズに到着できます。",
      ogDescription: "W3 Hostel の連絡先、交通情報、地図案内を確認できます。",
      title: "連絡先とアクセス",
      desc: "電車でも車でもアクセスしやすく、電話・メール・SNSからもお気軽にお問い合わせいただけます。",
      blendTitle: "W3 Hostel",
      blendDesc: "宜蘭県羅東鎮羅莊北街13号。公共交通でも車でもアクセスしやすい立地です。",
      cardTitle: "Contact",
      phone: "電話番号",
      address: "住所",
      email: "メール",
      transport: "公共交通",
      drive: "車でのアクセス",
      transportDesc: "羅東駅またはバスターミナルから徒歩約8〜10分です。",
      driveDesc: "国道5号から宜蘭方面へ進み、ナビで宜蘭県羅東鎮羅莊北街13号を目的地に設定してください。",
      map: "地図を見る",
      nav: "カーナビ"
    },

    footer: { rights: "W3 Hostel Copyright © 2026" },

    booking: {
      title: "クイックサービス",
      agoda: "Agodaで予約",
      booking: "Booking.comで予約",
      contact: "お問い合わせ",
      phone: "今すぐ電話",
      line: "LINE相談",
      ig: "Instagram"
    }
  },

  "ko": {
    nav: ["홈", "W3", "공간 및 시설", "여행 정보", "연락 및 오시는 길"],
    home: {
      metaTitle: "뤄동 숙소 추천｜W3 호스텔 이란｜야시장·역 근처",
      metaDescription: "W3 호스텔은 이란 뤄동 중심에 위치해 야시장과 역 인근에 있으며, 깔끔하고 조용한 숙박 환경을 제공합니다. 혼자 여행부터 단체 여행까지 모두에 적합합니다.",
      ogTitle: "W3 Hostel Luodong｜Clean & Quiet Stay",
      ogDescription: "Night Market nearby, full delivery coverage, and a flexible white-aesthetic stay in Luodong.",
      eyebrow: "뤄동 숙소 추천｜이란 호스텔",
      title: "뤄동에서 가장 잘 맞는 머무름을 찾으세요.",
      desc: "W3 호스텔은 뤄동 중심 생활권에 위치해 야시장, 기차역, 생활 편의시설과 가깝습니다. 배달이 편리하고, 공간은 조용하고 깔끔하며, 혼자 여행이나 단기 숙박, 친구와의 여행에도 잘 어울립니다.",
      cta1: "Agoda 예약",
      cta2: "Booking.com 예약",
      sec1k: "W3 숙박 경험",
      sec1t: "왜 W3를 선택할까요?",
      sec1d: "위치, 먹거리 접근성, 편안한 공간. W3의 핵심 장점을 세 가지로 소개합니다.",
      f1t: "중심 입지・도보 생활권",
      f1d: "뤄동 야시장, 기차역, 버스터미널, 편의점, 식당가가 가까워 이동과 식사가 편리합니다.",
      f2t: "배달 완전 지원",
      f2d: "Uber Eats와 foodpanda 이용이 가능해 인기 있는 뤄동 음식과 야식도 쉽게 즐길 수 있습니다.",
      f3t: "깔끔하고 조용한 공간",
      f3d: "밝고 정돈된 분위기에서 휴식, 독서, 업무, 여유로운 시간을 모두 누릴 수 있습니다.",
      sec2k: "숙박 방식",
      sec2t: "여행의 리듬은 당신이 정합니다.",
      sec2d: "낮에는 뤄동을 자유롭게 둘러보고, 밤에는 배달 음식을 즐기며, 쉬고 싶을 때는 조용하고 편안한 공간으로 돌아오세요.",
      x1t: "단기 워크스테이",
      x1d: "가벼운 업무, 일정 정리, 메시지 확인, 독서에 잘 맞습니다.",
      x2t: "편하게 식사하기",
      x2d: "줄 설 필요도, 밖에 나갈 필요도 없이 객실이나 공용 공간에서 뤄동 인기 음식을 즐길 수 있습니다.",
      x3t: "자연스럽게 쉬기",
      x3d: "서두르지 않고, 방해받지 않으며, 내 페이스대로 머물 수 있습니다.",
      sec3k: "위치의 장점",
      sec3t: "뤄동 중심에 머물면 모든 것이 더 간단해집니다.",
      sec3d: "교통, 식사, 생활 편의시설이 가까워 이동과 선택이 훨씬 수월합니다.",
      l1t: "뤄동 버스터미널",
      l1d: "차로 약 3분｜도보 약 13분",
      l2t: "뤄동 기차역",
      l2d: "차로 약 5분｜도보 약 18분",
      l3t: "뤄동 야시장",
      l3d: "차로 약 6분｜도보 약 20분 이상",
      sec4k: "지금 예약",
      sec4t: "뤄동에서의 숙박을 시작할 준비가 되셨나요?",
      sec4d: "객실 상황과 요금을 확인하고, 원하는 플랫폼에서 바로 예약하세요.",
      book1: "Agoda 예약 가능 여부 확인",
      book1d: "날짜, 객실 상태, 요금을 빠르게 확인하세요.",
      book2: "Booking.com 예약",
      book2d: "예약 페이지로 이동해 바로 예약을 완료하세요."
    },

    about: {
      metaTitle: "뤄동 W3 호스텔｜이란 중심 숙소・교통 편리",
      metaDescription: "뤄동 중심에 위치한 W3는 교통과 생활 편의성이 뛰어나며 조용하고 편안한 숙박 환경을 제공합니다.",
      ogTitle: "뤄동 W3｜도시 안에서 나만의 리듬과 공간 찾기",
      ogDescription: "교통이 편리하고 생활 인프라와 배달 환경이 잘 갖춰진 뤄동 중심 숙소입니다.",
      heroTitle: "뤄동 W3｜도시 안에서 나만의 리듬과 공간 찾기",
      heroDesc: "뤄동 핵심 생활권에 위치해 교통, 생활 편의, 배달 접근성이 모두 뛰어나며, 일과 휴식, 여유를 자연스럽게 오갈 수 있는 숙소입니다.",
      p1t: "입지 장점｜편리한 교통",
      p1d: "뤄동 기차역에서 차로 약 5–8분, 도보 약 15분 거리이며, 버스터미널에서는 차로 약 5분 또는 도보 약 12–15분 정도입니다. 타이베이에서 직행 기차나 버스로 접근하기도 쉽습니다.",
      p2t: "생활 편의｜완성된 생활권",
      p2d: "스시로, POYA, PX Mart, 7-Eleven, FamilyMart, 아침식당, 레스토랑, 음료 매장이 가까워 식사와 장보기가 편리합니다.",
      p3t: "배달의 장점｜핵심 매력",
      p3d: "Uber Eats와 foodpanda가 전 지역을 커버해 뤄동 야시장과 인기 식당 음식도 대부분 객실까지 배달받을 수 있습니다.",
      c1t: "입지 장점",
      c1i1: "뤄동 기차역: 차로 약 5–8분 / 도보 약 15분",
      c1i2: "뤄동 버스터미널: 차로 약 5분 / 도보 약 12–15분",
      c1i3: "타이베이에서 직행 기차 또는 버스로 접근 가능",
      c2t: "생활 편의",
      c2i1: "스시로, POYA, PX Mart가 근처에 있음",
      c2i2: "7-Eleven과 FamilyMart 밀집",
      c2i3: "아침식당, 식당, 음료 매장 도보권",
      c2i4: "식사와 생필품, 보충이 모두 편리함",
      c3t: "배달의 장점",
      c3i1: "Uber Eats / foodpanda 전 지역 지원",
      c3i2: "뤄동 인기 음식 대부분 배달 가능",
      c3i3: "줄 설 필요도, 외출할 필요도 없음",
      c3i4: "숙소 안에서 뤄동 먹거리를 즐길 수 있음"
    },

    space: {
      metaTitle: "공간 및 시설｜뤄동 W3 호스텔",
      metaDescription: "편안한 침대, 무료 세탁, 공용 주방과 조용한 공간으로 쾌적한 숙박 환경을 제공합니다.",
  ogTitle: "공간 및 시설｜뤄동 W3",
  ogDescription: "업무, 휴식, 프라이버시, 조용한 시간을 모두 담을 수 있는 유연한 숙소 공간입니다.",
  heroTitle: "공간 및 시설",
  heroDesc: "숙박 형태부터 생활 편의, 공용 공간, 주차까지 W3는 다양한 여행 리듬에 맞게 활용할 수 있는 공간을 제공합니다.",
  p1t: "숙박 형태",
  p1d: "침대 공간은 편안함과 기본적인 프라이버시를 제공하여 여행 중에도 자신의 리듬을 유지할 수 있도록 설계되었습니다.",
  p2t: "생활 편의",
  p2d: "무료 세탁기와 햇빛이 잘 드는 건조 공간이 있어 여행 중에도 옷을 세탁하고 정리하기 쉽습니다.",
  p3t: "공용 공간",
  p3d: "오픈 키친과 아일랜드 공간에는 2구 IH 조리기, 전자레인지, 오븐이 갖춰져 있어 간단한 조리와 데우기가 가능합니다. 독서 공간은 비교적 조용해 편안하게 머물 수 있습니다.",
  p4t: "공간의 성격",
  p4d: "업무, 휴식, 프라이버시, 조용한 시간 모두에 어울리며, 그때그때의 상태에 따라 공간 활용 방식을 바꿀 수 있습니다.",
  galleryTitle: "갤러리",
  galleryDesc: "빛과 여백 사이에서 W3 공간의 흐름을 천천히 느껴보세요."
},

travel: {
      metaTitle: "뤄동 여행 정보｜이란 관광・야시장",
      metaDescription: "뤄동 야시장과 주변 명소, 이란 자연 관광지를 정리해 여행 계획을 쉽게 도와드립니다.",
  ogTitle: "여행 정보｜뤄동 W3",
  ogDescription: "야시장부터 계절 행사와 자연 여행까지, W3를 거점으로 더 쉽게 여행을 계획할 수 있습니다.",
  heroTitle: "여행 정보",
  heroDesc: "주변 명소, 계절 행사, 자연 여행까지 뤄동과 이란 여행을 더 쉽게 계획할 수 있습니다.",
  p1t: "주변 관광지",
  p1d: "W3에 머물며 뤄동의 인기 명소와 지역 분위기를 편하게 즐길 수 있습니다.",
  p2t: "계절 하이라이트",
  p2d: "봄의 벚꽃, 여름의 강변, 가을의 논 풍경과 지역 행사까지 계절마다 다른 매력을 경험할 수 있습니다.",
  p3t: "이란 자연 탐방",
  p3d: "숲, 냉천, 지열, 산악 풍경 등 자연 여행에 잘 어울리는 풍경이 펼쳐집니다.",

  p4t: "가까운 명소",
  p4c1n: "뤄동 야시장",
  p4c1m: "🚖 6분 ・ 🚶 20분",
  p4c2n: "뤄동 임업문화원구",
  p4c2m: "🚖 7분 ・ 🚶 28분",
  p4c3n: "둥산허 친수공원",
  p4c3m: "🚖 9분",
  p4c4n: "뤄동 운동공원",
  p4c4m: "🚖 12분",
  p4c5n: "이란 전통예술센터",
  p4c5m: "🚖 12분",

  p5t: "계절 행사 카니발",
  p5c1n: "🌸 뤄좡 벚꽃 시즌",
  p5c1m: "매년 2월 중순~3월 초, 뤄좡 벚꽃 산책길, 🚖 2분 ・ 🚶 10분",
  p5c2n: "🌾 뤄동 벼이삭 축제",
  p5c2m: "매년 10~11월, 뤄동 주변 논밭, 🚶 약 30초",
  p5c3n: "🌿 브라운 대로",
  p5c3m: "연중 방문 가능, 191번 현도, 🚖 약 10분",
  p5c4n: "🎆 춘췌 국제온천호텔 연말 불꽃놀이",
  p5c4m: "매년 12월 31일, 🚖 3분 ・ 🚶 11분",

  p6t: "자연 여행 확장 코스",
  p6c1n: "타이핑산 국가삼림유락구",
  p6c1m: "🚖 90분",
  p6c2n: "쑤아오 냉천공원",
  p6c2m: "🚖 21분"
},

    contact: {
      metaTitle: "연락 및 위치｜뤄동 W3 호스텔",
      metaDescription: "주소, 전화번호, 교통 정보를 확인하고 쉽게 찾아오세요.",
      ogTitle: "연락 및 오시는 길｜뤄동 W3",
      ogDescription: "W3 호스텔의 연락처, 교통 정보, 지도 안내를 확인할 수 있습니다.",
      title: "연락 및 오시는 길",
      desc: "대중교통이나 자가용 모두 편리하며, 전화, 이메일, SNS로도 쉽게 문의하실 수 있습니다.",
      blendTitle: "W3 Hostel",
      blendDesc: "이란현 뤄동진 뤄좡북가 13호. 대중교통과 자가용 모두 편리하게 접근할 수 있습니다.",
      cardTitle: "Contact",
      phone: "전화",
      address: "주소",
      email: "이메일",
      transport: "대중교통",
      drive: "자가용",
      transportDesc: "뤄동 기차역 또는 버스터미널에서 도보 약 8–10분 거리입니다.",
      driveDesc: "국도 5호선을 통해 이란으로 진입한 뒤, 내비게이션에 이란현 뤄동진 뤄좡북가 13호를 입력하세요.",
      map: "지도 보기",
      nav: "길찾기"
    },

    footer: { rights: "W3 Hostel © 2026" },

    booking: {
      title: "빠른 서비스",
      agoda: "Agoda 예약",
      booking: "Booking.com 예약",
      contact: "문의하기",
      phone: "전화하기",
      line: "LINE 상담",
      ig: "Instagram"
    }
  }
};

function setLang(lang) {
  const t = translations[lang] || translations["zh-Hant"];
  document.documentElement.lang = lang;
  localStorage.setItem("w3-lang", lang);

  const params = new URLSearchParams(window.location.search);
  params.set("lang", lang);
  history.replaceState(null, "", `${window.location.pathname}?${params.toString()}`);

  document.querySelectorAll("[data-i18n]").forEach(el => {
    const path = el.dataset.i18n.split(".");
    let value = t;
    path.forEach(k => value = value?.[k]);
    if (typeof value === "string") el.textContent = value;
  });

  document.querySelectorAll("[data-nav-index]").forEach(el => {
    const idx = Number(el.dataset.navIndex);
    if (t.nav[idx]) el.textContent = t.nav[idx];
  });

  document.querySelectorAll(".lang-switch button").forEach(btn => {
    btn.classList.toggle("active", btn.dataset.lang === lang);
  });

  applyMetaByPage(t);
}

function applyMetaByPage(t) {
  const path = window.location.pathname;
  let pageKey = "home";

  if (path.endsWith("/w3.html") || path.endsWith("w3.html")) pageKey = "about";
  else if (path.endsWith("/space.html") || path.endsWith("space.html")) pageKey = "space";
  else if (path.endsWith("/travel.html") || path.endsWith("travel.html")) pageKey = "travel";
  else if (path.endsWith("/contact.html") || path.endsWith("contact.html")) pageKey = "contact";

  const page = t[pageKey];
  if (!page) return;

  if (page.metaTitle) document.title = page.metaTitle;

  const metaDesc = document.querySelector('meta[name="description"]');
  if (metaDesc && page.metaDescription) metaDesc.setAttribute("content", page.metaDescription);

  const ogTitle = document.querySelector('meta[property="og:title"]');
  if (ogTitle && page.ogTitle) ogTitle.setAttribute("content", page.ogTitle);

  const ogDesc = document.querySelector('meta[property="og:description"]');
  if (ogDesc && page.ogDescription) ogDesc.setAttribute("content", page.ogDescription);
}

function injectFloatingBooking() {
  if (document.querySelector(".floating-actions")) return;

  const widget = document.createElement("div");
  widget.className = "floating-actions";
  widget.innerHTML = `
    <button class="fa-main" type="button" aria-expanded="false" aria-controls="quick-panel">
      <span class="fa-icon">✦</span>
      <span class="fa-label">
        <span data-i18n="booking.title">快捷服務</span>
        <small>BOOK · CALL · LINE · IG</small>
      </span>
    </button>
    <div class="fa-panel" id="quick-panel">
      <a class="action-link book" href="https://www.agoda.com/backpacker-s-hostel-luodong/hotel/yilan-tw.html" target="_blank" rel="noopener">
        <span data-i18n="booking.agoda">Agoda 訂房</span>
        <span class="meta">Open</span>
      </a>
      <a class="action-link book" href="https://www.booking.com/hotel/tw/w3-hostel.zh-tw.html" target="_blank" rel="noopener">
        <span data-i18n="booking.booking">Booking.com 訂房</span>
        <span class="meta">Open</span>
      </a>
      <a class="action-link phone" href="tel:+886937096423">
        <span data-i18n="booking.phone">立即撥打</span>
        <span class="meta">+886 937 096 423</span>
      </a>
      <a class="action-link line" href="https://line.me/R/ti/p/@749azqqe" target="_blank" rel="noopener">
        <span data-i18n="booking.line">LINE 諮詢</span>
        <span class="meta">LINE</span>
      </a>
      <a class="action-link ig" href="https://www.instagram.com/w3_hostel/" target="_blank" rel="noopener">
        <span data-i18n="booking.ig">Instagram</span>
        <span class="meta">@w3_hostel</span>
      </a>
    </div>
  `;
  document.body.appendChild(widget);

  const mainBtn = widget.querySelector(".fa-main");
  mainBtn.addEventListener("click", () => {
    const open = widget.classList.toggle("open");
    mainBtn.setAttribute("aria-expanded", open ? "true" : "false");
  });

  document.addEventListener("click", (e) => {
    if (!widget.contains(e.target)) {
      widget.classList.remove("open");
      mainBtn.setAttribute("aria-expanded", "false");
    }
  });

  let lastY = window.scrollY || 0;
  window.addEventListener("scroll", () => {
    const y = window.scrollY || 0;
    const nearTop = y < 80;
    const scrollingDown = y > lastY && y > 180;
    widget.classList.toggle("is-hidden", scrollingDown && !widget.classList.contains("open") && !nearTop);
    lastY = y;
  }, { passive: true });
}

document.addEventListener("DOMContentLoaded", () => {
  const urlLang = new URLSearchParams(window.location.search).get("lang");
  const saved = urlLang || localStorage.getItem("w3-lang") || "zh-Hant";
  setLang(saved);
  injectFloatingBooking();

  document.querySelectorAll(".lang-switch button").forEach(btn => {
    btn.addEventListener("click", () => setLang(btn.dataset.lang));
  });

  const revealObserver = new IntersectionObserver(entries => {
    entries.forEach(entry => {
      if (entry.isIntersecting) entry.target.classList.add("visible");
    });
  }, { threshold: 0.12 });

  document.querySelectorAll(".reveal").forEach(el => revealObserver.observe(el));

  const topbar = document.querySelector(".topbar");
  const sceneEls = document.querySelectorAll(".page-hero, .parallax, .multi-bg, .blend-bg, .card, .section-head");
  const hero = document.querySelector(".hero");
  const heroSlides = document.querySelector(".hero .slides");
  const heroContent = document.querySelector(".hero-content");
  const pageHero = document.querySelector(".page-hero");
  const pageHeroInner = document.querySelector(".page-hero .container");
  const reducedMotion = window.matchMedia("(prefers-reduced-motion: reduce)").matches;

  document.querySelectorAll("main section").forEach((section, idx, arr) => {
    if (idx < arr.length - 1 && !section.nextElementSibling?.classList?.contains("scene-divider")) {
      const divider = document.createElement("div");
      divider.className = "scene-divider";
      section.insertAdjacentElement("afterend", divider);
    }
  });

  function clamp(value, min, max) {
    return Math.min(Math.max(value, min), max);
  }

  function updateScenes() {
    const scrollY = window.scrollY || window.pageYOffset;
    const viewportH = window.innerHeight || 1;
    document.documentElement.style.setProperty("--scroll-y", `${scrollY}px`);

    if (topbar) topbar.classList.toggle("is-scrolled", scrollY > 12);

    if (!reducedMotion && hero) {
      const hr = hero.getBoundingClientRect();
      const progress = clamp((viewportH - hr.top) / (viewportH + hr.height), 0, 1.25);
      const shift = Math.round(progress * 110);
      const scale = 1 + progress * 0.06;
      document.documentElement.style.setProperty("--hero-shift", `${shift}px`);
      document.documentElement.style.setProperty("--hero-scale", scale.toFixed(4));
      document.documentElement.style.setProperty("--hero-content-shift", `${Math.round(progress * 56)}px`);
      if (heroSlides) heroSlides.style.opacity = `${1 - progress * 0.18}`;
      if (heroContent) heroContent.style.opacity = `${1 - progress * 0.1}`;
    }

    if (!reducedMotion && pageHero) {
      const pr = pageHero.getBoundingClientRect();
      const progress = clamp((viewportH - pr.top) / (viewportH + pr.height), 0, 1.3);
      const shift = Math.round(progress * 72);
      const scale = 1 + progress * 0.03;
      document.documentElement.style.setProperty("--hero-shift", `${shift}px`);
      document.documentElement.style.setProperty("--hero-scale", scale.toFixed(4));
      document.documentElement.style.setProperty("--hero-content-shift", `${Math.round(progress * 18)}px`);
      if (pageHeroInner) pageHeroInner.style.opacity = `${1 - progress * 0.08}`;
    }

    sceneEls.forEach(el => {
      const r = el.getBoundingClientRect();
      const center = r.top + r.height / 2;
      const distance = (viewportH / 2 - center) / viewportH;
      const depth = clamp(1 - Math.abs(distance * 2.1), 0, 1);
      const shift = Math.round(distance * 90);
      const tilt = (distance * 7).toFixed(2);

      el.style.setProperty("--scene-depth", depth.toFixed(3));
      el.style.setProperty("--scene-shift", `${shift}px`);
      el.style.setProperty("--scene-tilt", `${tilt}deg`);
      el.classList.toggle("scene-active", depth > 0.02);
    });
  };

  updateScenes();
  window.addEventListener("scroll", updateScenes, { passive: true });
  window.addEventListener("resize", updateScenes);
});

function setLang(lang) {
  const t = translations[lang] || translations["zh-Hant"];
  document.documentElement.lang = lang;
  localStorage.setItem("w3-lang", lang);

  const params = new URLSearchParams(window.location.search);
  params.set("lang", lang);
  history.replaceState(null, "", `${window.location.pathname}?${params.toString()}`);

  document.querySelectorAll("[data-i18n]").forEach(el => {
    const path = el.dataset.i18n.split(".");
    let value = t;
    path.forEach(k => value = value?.[k]);
    if (typeof value === "string") el.textContent = value;
  });

  document.querySelectorAll("[data-nav-index]").forEach(el => {
    const idx = Number(el.dataset.navIndex);
    if (t.nav[idx]) el.textContent = t.nav[idx];
  });

  document.querySelectorAll(".lang-switch button").forEach(btn => {
    btn.classList.toggle("active", btn.dataset.lang === lang);
  });

  applyMetaByPage(t);
}

function applyMetaByPage(t) {
  const path = window.location.pathname;
  let pageKey = "home";

  if (path.endsWith("/w3.html") || path.endsWith("w3.html")) pageKey = "about";
  else if (path.endsWith("/space.html") || path.endsWith("space.html")) pageKey = "space";
  else if (path.endsWith("/travel.html") || path.endsWith("travel.html")) pageKey = "travel";
  else if (path.endsWith("/contact.html") || path.endsWith("contact.html")) pageKey = "contact";

  const page = t[pageKey];
  if (!page) return;

  if (page.metaTitle) document.title = page.metaTitle;

  const metaDesc = document.querySelector('meta[name="description"]');
  if (metaDesc && page.metaDescription) metaDesc.setAttribute("content", page.metaDescription);

  const ogTitle = document.querySelector('meta[property="og:title"]');
  if (ogTitle && page.ogTitle) ogTitle.setAttribute("content", page.ogTitle);

  const ogDesc = document.querySelector('meta[property="og:description"]');
  if (ogDesc && page.ogDescription) ogDesc.setAttribute("content", page.ogDescription);
}

function injectFloatingBooking() {
  if (document.querySelector(".floating-actions")) return;

  const widget = document.createElement("div");
  widget.className = "floating-actions";
  widget.innerHTML = `
    <button class="fa-main" type="button" aria-expanded="false" aria-controls="quick-panel">
      <span class="fa-icon">✦</span>
      <span class="fa-label">
        <span data-i18n="booking.title">快捷服務</span>
        <small>BOOK · CALL · LINE · IG</small>
      </span>
    </button>
    <div class="fa-panel" id="quick-panel">
      <a class="action-link book" href="https://www.agoda.com/backpacker-s-hostel-luodong/hotel/yilan-tw.html" target="_blank" rel="noopener">
        <span data-i18n="booking.agoda">Agoda 訂房</span>
        <span class="meta">Open</span>
      </a>
      <a class="action-link book" href="https://www.booking.com/hotel/tw/w3-hostel.zh-tw.html" target="_blank" rel="noopener">
        <span data-i18n="booking.booking">Booking.com 訂房</span>
        <span class="meta">Open</span>
      </a>
      <a class="action-link phone" href="tel:+886937096423">
        <span data-i18n="booking.phone">立即撥打</span>
        <span class="meta">+886 937 096 423</span>
      </a>
      <a class="action-link line" href="https://line.me/R/ti/p/@749azqqe" target="_blank" rel="noopener">
        <span data-i18n="booking.line">LINE 諮詢</span>
        <span class="meta">LINE</span>
      </a>
      <a class="action-link ig" href="https://www.instagram.com/w3_hostel/" target="_blank" rel="noopener">
        <span data-i18n="booking.ig">Instagram</span>
        <span class="meta">@w3_hostel</span>
      </a>
    </div>
  `;
  document.body.appendChild(widget);

  const mainBtn = widget.querySelector(".fa-main");
  mainBtn.addEventListener("click", () => {
    const open = widget.classList.toggle("open");
    mainBtn.setAttribute("aria-expanded", open ? "true" : "false");
  });

  document.addEventListener("click", (e) => {
    if (!widget.contains(e.target)) {
      widget.classList.remove("open");
      mainBtn.setAttribute("aria-expanded", "false");
    }
  });

  let lastY = window.scrollY || 0;
  window.addEventListener("scroll", () => {
    const y = window.scrollY || 0;
    const nearTop = y < 80;
    const scrollingDown = y > lastY && y > 180;
    widget.classList.toggle("is-hidden", scrollingDown && !widget.classList.contains("open") && !nearTop);
    lastY = y;
  }, { passive: true });
}

document.addEventListener("DOMContentLoaded", () => {
  const urlLang = new URLSearchParams(window.location.search).get("lang");
  const saved = urlLang || localStorage.getItem("w3-lang") || "zh-Hant";
  setLang(saved);
  injectFloatingBooking();

  document.querySelectorAll(".lang-switch button").forEach(btn => {
    btn.addEventListener("click", () => setLang(btn.dataset.lang));
  });

  const revealObserver = new IntersectionObserver(entries => {
    entries.forEach(entry => {
      if (entry.isIntersecting) entry.target.classList.add("visible");
    });
  }, { threshold: 0.12 });

  document.querySelectorAll(".reveal").forEach(el => revealObserver.observe(el));

  const topbar = document.querySelector(".topbar");
  const sceneEls = document.querySelectorAll(".page-hero, .parallax, .multi-bg, .blend-bg, .card, .section-head");
  const hero = document.querySelector(".hero");
  const heroSlides = document.querySelector(".hero .slides");
  const heroContent = document.querySelector(".hero-content");
  const pageHero = document.querySelector(".page-hero");
  const pageHeroInner = document.querySelector(".page-hero .container");
  const reducedMotion = window.matchMedia("(prefers-reduced-motion: reduce)").matches;

  document.querySelectorAll("main section").forEach((section, idx, arr) => {
    if (idx < arr.length - 1 && !section.nextElementSibling?.classList?.contains("scene-divider")) {
      const divider = document.createElement("div");
      divider.className = "scene-divider";
      section.insertAdjacentElement("afterend", divider);
    }
  });

  function clamp(value, min, max) {
    return Math.min(Math.max(value, min), max);
  }

  function updateScenes() {
    const scrollY = window.scrollY || window.pageYOffset;
    const viewportH = window.innerHeight || 1;
    document.documentElement.style.setProperty("--scroll-y", `${scrollY}px`);

    if (topbar) topbar.classList.toggle("is-scrolled", scrollY > 12);

    if (!reducedMotion && hero) {
      const hr = hero.getBoundingClientRect();
      const progress = clamp((viewportH - hr.top) / (viewportH + hr.height), 0, 1.25);
      const shift = Math.round(progress * 110);
      const scale = 1 + progress * 0.06;
      document.documentElement.style.setProperty("--hero-shift", `${shift}px`);
      document.documentElement.style.setProperty("--hero-scale", scale.toFixed(4));
      document.documentElement.style.setProperty("--hero-content-shift", `${Math.round(progress * 56)}px`);
      if (heroSlides) heroSlides.style.opacity = `${1 - progress * 0.18}`;
      if (heroContent) heroContent.style.opacity = `${1 - progress * 0.1}`;
    }

    if (!reducedMotion && pageHero) {
      const pr = pageHero.getBoundingClientRect();
      const progress = clamp((viewportH - pr.top) / (viewportH + pr.height), 0, 1.3);
      const shift = Math.round(progress * 72);
      const scale = 1 + progress * 0.03;
      document.documentElement.style.setProperty("--hero-shift", `${shift}px`);
      document.documentElement.style.setProperty("--hero-scale", scale.toFixed(4));
      document.documentElement.style.setProperty("--hero-content-shift", `${Math.round(progress * 18)}px`);
      if (pageHeroInner) pageHeroInner.style.opacity = `${1 - progress * 0.08}`;
    }

    sceneEls.forEach(el => {
      const r = el.getBoundingClientRect();
      const center = r.top + r.height / 2;
      const distance = (viewportH / 2 - center) / viewportH;
      const depth = clamp(1 - Math.abs(distance * 2.1), 0, 1);
      const shift = Math.round(distance * 90);
      const tilt = (distance * 7).toFixed(2);

      el.style.setProperty("--scene-depth", depth.toFixed(3));
      el.style.setProperty("--scene-shift", `${shift}px`);
      el.style.setProperty("--scene-tilt", `${tilt}deg`);
      el.classList.toggle("scene-active", depth > 0.02);
    });
  }

  updateScenes();
  window.addEventListener("scroll", updateScenes, { passive: true });
  window.addEventListener("resize", updateScenes);
  document.querySelectorAll(".parallax-carousel").forEach(carousel => {
  
  const slides = carousel.querySelectorAll(".parallax-slide");
  if (slides.length <= 1) return;

  let current = 0;

  setInterval(() => {
    slides[current].classList.remove("active");
    current = (current + 1) % slides.length;
    slides[current].classList.add("active");
  }, 4000);
});
});